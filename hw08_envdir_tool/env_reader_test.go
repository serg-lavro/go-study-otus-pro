...
func TestReadDir(t *testing.T) {
	t.Run("simple file", func(t *testing.T) {
		err := os.MkdirAll("testdata/testenv", 0777)
		if err != nil {
			log.Fatal(err)
			return
		}
		...
		f, err := os.OpenFile("testdata/testenv/VAR", os.O_RDWR|os.O_CREATE, 0644)
		if err != nil {
			log.Fatal(err)
			return
		}
	...
	t.Run("simple three files", func(t *testing.T) {
		err := os.MkdirAll("testdata/testenv", 0777)
		if err != nil {
			log.Fatal(err)
			return
		...
		f1, err := os.OpenFile("testdata/testenv/VAR1", os.O_RDWR|os.O_CREATE, 0644)
		if err != nil {
			log.Fatal(err)
			return
		}
		...
		defer f1.Close()
		f2, err := os.OpenFile("testdata/testenv/VAR2", os.O_RDWR|os.O_CREATE, 0644)
		if err != nil {
			log.Fatal(err)
			return
		}
		defer os.Remove("testdata/testenv/VAR2")
		...
		defer f2.Close()
		f3, err := os.OpenFile("testdata/testenv/VAR3", os.O_RDWR|os.O_CREATE, 0644)
		if err != nil {
			log.Fatal(err)
			return
		}
		...
		got, _ := ReadDir("testdata/testenv")
		want := Environment{"VAR1": {"var1", false},
							"VAR2": {"var2", false},
							"VAR3": {"var3", false}}
		eq := reflect.DeepEqual(want, got)
		if !eq {
	...
	t.Run("no = in file name", func(t *testing.T) {
		...
		defer f1.Close()
		f2, err := os.OpenFile("testdata/testenv/VAR=2", os.O_RDWR|os.O_CREATE, 0644)
		if err != nil {
			log.Fatal(err)
			return
		}
